...
import useProductUpdateState from 'hooks/useProductUpdateState';
import ProductButtonAction from './product-button-action';
import timeAgoDAYJS, { dateFormat } from 'helpers/timeAgoDAYJS';
  ...
  return (
    <>
      <li
        ...
        } ${item.state === 'ACTIVE' ? 'border-yellow-500' : 'border-gray-300'}`}>
        <button
          className="text-gray-400 px-1 border-r border-gray-300"
          onClick={() => productUpdate(item._id, { state: 'SOLD' })}
          disabled={loadingUpdate}>
          {loadingUpdate ? (
            <LoadingIcon h="15" w="15" classN="mx-small" fill="#90A4AE" />
          ) : (
            <svg
              className="w-5 h-5"
              fill="currentColor"
        ...
        <div className="section-main grid col-gap-1 p-2 relative">
          ...
          <div className="grid text-gray-400">
            {item.status === 'PROMOTION' ? (
              ...
              <svg
                className={`w-4 h-4 mt-small ${item.status === 'NEW' && 'text-green-500'}`}
                fill="currentColor"
                viewBox="0 0 20 20"
                xmlns="http://www.w3.org/2000/svg">
                <path
        ...
        <div
          ...
          <p
            ...
            title={`Creado: ${dateFormat(item.createdAt)}`}>
            {timeAgoDAYJS(item.createdAt)}
          </p>
          <div className="button-actions flex space-x-2 items-center justify-end">
            ...
            <ProductButtonAction
              ...
              onClick={() => productUpdateState(item._id)}
              colorText={item.state === 'STOCK' ? 'text-alain-blue-500' : ''}>
              {loadingUpdateState ? <LoadingIcon h="16" w="16" fill="#90A4AE" /> : state}
            </ProductButtonAction>
          </div>
        </div>
      </li>
      <ComponentPortalRender render={loadingUpdateState || loadingDestroy}>
