export default function NavBar(){

  // TODO · Dividir en componente reutilizable 10/05/2020 

  return (
    <>
      <footer className="sticky bottom-0 left-0 w-full bg-white grid place-items-center">

        <nav className="w-full px-4">
          <ul className="flex justify-around">
            <li>
              <a href="#" className="w-10 h-10 text-gray-500 rounded-full grid place-items-center">
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z" /></svg>
              </a>
            </li>
            <li>
              <a href="#" className="w-10 h-10 text-gray-500 rounded-full grid place-items-center">
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M5 3a2 2 0 00-2 2v2a2 2 0 002 2h2a2 2 0 002-2V5a2 2 0 00-2-2H5zM5 11a2 2 0 00-2 2v2a2 2 0 002 2h2a2 2 0 002-2v-2a2 2 0 00-2-2H5zM11 5a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V5zM11 13a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>
              </a>
            </li>
            <li>
              <a href="#" className="w-10 h-10 bg-alain-blue-500 text-white rounded-full grid place-items-center shadow-alain">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
              </a>
            </li>
            <li>
              <a href="#" className="w-10 h-10 text-gray-500 rounded-full grid place-items-center">
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M3 3a1 1 0 000 2v8a2 2 0 002 2h2.586l-1.293 1.293a1 1 0 101.414 1.414L10 15.414l2.293 2.293a1 1 0 001.414-1.414L12.414 15H15a2 2 0 002-2V5a1 1 0 100-2H3zm11.707 4.707a1 1 0 00-1.414-1.414L10 9.586 8.707 8.293a1 1 0 00-1.414 0l-2 2a1 1 0 101.414 1.414L8 10.414l1.293 1.293a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
              </a>
            </li>
            <li>
              <a href="#" className="w-10 h-10 text-gray-500 rounded-full grid place-items-center border border-gray-200">
                <img src="/perfil-default-img.png" alt="Avatar small"/>
              </a>
            </li>
          </ul>
        </nav>
  
      </footer>
      <style jsx>{`
        footer{
          height: 56px;
          -webkit-box-shadow: 0px -2px 4px 0px rgba(	30, 27, 77,0.13); 
          box-shadow: 0px -2px 4px 0px rgba(	30, 27, 77,0.13);
        }  
      `}</style>
    </>
  );
};